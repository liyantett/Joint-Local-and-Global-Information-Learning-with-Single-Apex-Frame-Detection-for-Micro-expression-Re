...
def im_detect(net, im, boxes, gt_label):
    ...
    #pool = net.blobs['context_pool5'].data[0,:512]
    ...
    selected_boxes = np.zeros((secondary_scores.shape[1], 4, gt_boxes.shape[0]))
    ...
    for i in xrange(gt_boxes.shape[0]):
        ...
        scores = np.amax(secondary_scores[keep_inds,:], axis=0)
        if keep_inds.size > 0:
    ...
    #print(this_scores)
    # Softmax     
    scores = np.exp(scores-np.amax(this_scores))
...
def test_net(net, imdb):
    ...
    for i in xrange(num_images):
        ...
        secondary_scores = np.exp(secondary_scores-np.amax(secondary_scores))
        secondary_scores=secondary_scores / (np.sum(secondary_scores))

        secondary_scores=secondary_scores/sum(secondary_scores)
        #print(secondary_scores)
	maxscore=np.max(secondary_scores)

        ind=np.argwhere(secondary_scores==maxscore)
        indy=ind[0][1]
        #print(indy)
