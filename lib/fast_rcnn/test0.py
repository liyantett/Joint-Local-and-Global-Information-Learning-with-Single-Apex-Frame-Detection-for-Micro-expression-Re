...
def im_detect(net, im, boxes, gt_label):
    ...
    blobs_sec_rois=blobs_sec_rois.reshape(num_sec_rois, 5, 1, 1)
    #print 'a'
    ...
    #zhixindu
    #scores_confidence=scores
    #scores_confidence=np.exp(scores_confidence-np.amax(scores_confidence))
    #scores_confidence = scores_confidence / np.array(np.sum(scores_confidence, axis=1), ndmin=2).T
    #scores=scores*np.amax(scores_confidence)
     
    
    #secondary_scores_confidence=secondary_scores
    #secondary_scores_confidence_length=len(secondary_scores_confidence)
    #print secondary_scores_confidence_length
    #for x in range(0,(secondary_scores_confidence_length-1)):
	#temp=secondary_scores_confidence[x]
        #temp=np.exp(temp-np.amax(temp,axis=0))       
        #temp = temp / np.array(np.sum(temp, axis=0), ndmin=2).T
	#secondary_scores_confidence[x]=temp
	
    #secondary_scores=secondary_scores*np.amax(secondary_scores_confidence)  
